...
export default function InterviewPage(): JSX.Element {
  ...
  const hasRoleLink = Boolean(roleState?.roleId)
  const [phase, setPhase] = useState<Phase>(() => {
    if (hasRoleLink) return 'setup'
    return initialState.current ? 'interview' : 'setup'
  })
  ...
  const [companyWebsite, setCompanyWebsite] = useState<string | null>(
    hasRoleLink ? null : initialState.current?.companyWebsite || null,
  )
  ...
  const [activeSessionId, setActiveSessionId] = useState<string | null>(
    hasRoleLink ? null : (initialState.current?.sessionId ?? null),
  )
  ...
  }, [resumeSession, roleState?.roleId])
  ...
  const lastCheckedRoleIdRef = useRef<string | null>(null)
  useEffect(() => {
    const roleId = roleState?.roleId
    if (!roleId) return
    if (lastCheckedRoleIdRef.current === roleId) return
    lastCheckedRoleIdRef.current = roleId

    resetSession()
    autoGeneratedForRef.current = null
    clearSession()

    setCompanyName(roleState?.companyName ?? '')
    setSelectedCompanyId(roleState?.companyProfileId ?? null)
    setCompanyRoleId(roleId)
    setCompanyProfile(null)
    setCompanyWebsite(null)
    setJobDescription(roleState?.jobDescription ?? '')
    setStyle('mixed')
    setActiveResume(null)
    setSelectedResumeId(null)
    ...
    void supabase
      ...
      .then(({ data, error }) => {
        ...
        void handleLoadSession({
          ...
          companyName: roleState?.companyName ?? '',
          companyProfileId: roleState?.companyProfileId ?? null,
    ...
    roleState?.companyProfileId,
    roleState?.jobDescription,
    roleState?.roleTitle,
    handleLoadSession,
    resetSession,
  ])
