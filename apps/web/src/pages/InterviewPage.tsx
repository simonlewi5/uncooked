...
export default function InterviewPage(): JSX.Element {
  ...
  const [isRestoringSession, setIsRestoringSession] = useState(
    Boolean(initialState.current?.sessionId),
  )
  ...
  useEffect(() => {
    if (interviewSessionId && interviewSessionId !== activeSessionId) {
      ...
      saveSession({
        ...
        style: style ?? 'mixed',
        sidebarWidth,
        questionCooldownEnd: questionCooldownEnd ?? undefined,
      })
    }
},[
    interviewSessionId,
  ...
  useEffect(() => {
    ...
    Promise.resolve(
      ...
      .then(({ data, error }) => {
        ...
        const loaded =
          (data.messages as Array<{ role: string; content: string; timestamp: string }> | null ?? [])
            .map((m, i) => ({
              id: `past-${i}`,
        ...
        setStyle((data.interview_style as InterviewStyle) ?? 'mixed')
        setPhase('interview')
        setIsRestoringSession(false)
      })
      .catch(() => {
    ...
    return (
      <div className={styles.setupPage}>
        <div className={styles.setupHeader}>
          ...
          <p className={styles.subtitle}>Restoring your interview session...</p>
        </div>
      </div>
    )
  }
